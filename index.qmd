...
- Determinar se pares de registros de dados descrevem a mesma entidade (geralmente pessoas, organizações, etc.), através de atributos desses registros, ex: CPF, CNS, Nome, Endereço, Data de Nascimento, etc.
...
## Fluxo do Linkage Probabilístico

![](figuras/Workflow.png){fig-align="center"}

## Fluxo do Linkage Probabilístico

- **Pré-processamento:** desenvolvimento de chaves de relacionamento, ex: concatenação de atributos (ex: nome + nome da mãe + data do nascimento) de forma direta ou via criptografia (ex: HASH MD5)

- **Redução do espaço de busca**: Comparação via estratégias de blocagem, ex: primeiro nome, ultimo nome, ano de nascimento, nome do bairro, etc.

- **Classificação:** Comparação dos atributos via medidas de similaridade

- **Preditor final:** pontos de corte dos scores e validação
